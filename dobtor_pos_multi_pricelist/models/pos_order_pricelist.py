...
class PosOrderPircelist(models.Model):
    ...
    discount_rule = fields.One2many(string='Pricelist Items', comodel_name='pos.order.pricelist.rule',inverse_name='pricelist_id')
    discount_item = fields.Many2one('product.product.discount','discount item',required=True,)
...
class PosOrderPircelistRule(models.Model):
    ...
    name = fields.Char(string='Name', required=True)
    pricelist_id = fields.Many2one(string='pricelist_id', comodel_name='pos.order.pricelist')
