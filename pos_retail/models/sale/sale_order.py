...
class sale_order(models.Model):
    ...
    def get_data(self):
        ...
        data = self.sudo().read(fields_sale_load)[0]
...
class sale_order_line(models.Model):
    ...
    variant_ids = fields.Many2many('product.variant', 'sale_line_variant_rel', 'sale_line_id', 'variant_id',
                                   string='Variants')
    pos_note = fields.Text('POS note')

    def get_data(self):
        cache_obj = self.env['pos.cache.database']
        fields_sale_load = cache_obj.get_fields_by_model(self._inherit)
        datas = self.sudo().read(fields_sale_load)
        if not datas:
            data = {}
            data['deleted'] = True
        else:
