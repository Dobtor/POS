...
class pos_config(models.Model):
    ...
    invoice_offline = fields.Boolean('Invoice offline', help='Help cashiers passing waiting time print invoice')
    wallet = fields.Boolean('Wallet customer')
    ...
    add_customer_before_products_already_in_shopping_cart = fields.Boolean('Required add client first',
                                                                           help='Add customer before products already in shopping cart',
                                                                           default=0)
    allow_cashier_select_pricelist = fields.Boolean('Allow cashier use pricelist', help='If uncheck, pricelist only work when select customer. Cashiers could not manual choice pricelist', default=1)
    use_parameters = fields.Boolean('Use parameters',
                                    help='POS need only one time save parameter datas use on POS, and next times no need call backend',
                                    default=0)
    time_refresh_parameter = fields.Integer('Time refresh datas (seconds)', help='Time for refresh parameters data',
                                            default=30)
    auto_sync = fields.Boolean('Auto sync', help='Auto sync modified backend to POS', default=1)
    ...
    sync_products = fields.Boolean('Sync products', default=1, help='Synchronize products between backend and pos')
    sync_customers = fields.Boolean('Sync customers', default=1, help='Synchronize customers between backend and pos')
    sync_invoices = fields.Boolean('Sync invoices', default=1, help='Synchronize invoices between backend and pos')
    sync_pos_orders = fields.Boolean('Sync pos order', default=1, help='Synchronize pos orders between backend and pos')
    sync_pricelist = fields.Boolean('Sync pricelists', default=1, help='Synchronize pricelists between backend and pos')
    allow_auto_rounding = fields.Boolean('Auto Rounding')
    rounding = fields.Float('Rounding', default=0.1)
    max_products_render = fields.Integer('Max products render', help='Maximum products render on pos screen',
                                         default=99999)
