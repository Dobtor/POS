    ...
    models.PosModel = models.PosModel.extend({
        ...
        store_pos_models: function () {
            ...
            for (var i = 0; i < this.models.length; i++) {
                ...
                if (model.model == 'res.partner' || model.model == 'product.product' || model.model == 'pos.order' || model.model == 'pos.order.line' || model.model == 'account.invoice') {
            ...
            return _super_PosModel.load_server_data.apply(this, arguments).then(function () {
                ...
                return cache_records.then(function (datas) {
                    ...
                    if (partners.length > 0 && product_ids.length > 0) {
                        ...
                        if (self.config.sync_stock == true && self.config.display_onhand == true) {
                            self.chrome.loading_message(_t('Waiting get products quantity available'), 1);
                            self.chrome.loading_show();
                            return $.when(rpc.query(params).then(function (qty_available_by_product_id) {
                                for (var i = 0; i < self.products.length; i++) {
                                    var product = self.products[i];
                                    product['qty_available'] = qty_available_by_product_id[product['id']] || 0;
                                }
                                return $.when(product_model.loaded(self, self.products, {})).then(function () {
                                });
                            }))
                        } else {
                        ...
                        if (product_model) { // if old data install pos_cache odoo
                            return $.when(self.load_cache(product_model)).then(function () {
                                return $.when(self.load_cache(partner_model)).then(function () {
                                    return $.when(self.load_cache(invoice_model)).then(function () {
                                        return $.when(self.load_cache(po_model)).then(function () {
                                            return $.when(self.load_cache(pol_model))
                                        })
                                    })
                                })
                            })
