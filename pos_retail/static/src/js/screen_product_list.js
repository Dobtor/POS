...
odoo.define('pos_retail.screen_product_list', function (require) {
    ...
    var qweb = core.qweb;
    var _t = core._t;
    var core_screen = require('pos_retail.product_category');
    ...
    screens.ProductScreenWidget.include({
                    ...
                    super_button.button_click = function () {
                        if (!self.pos.config.allow_cashier_select_pricelist) {
                            return self.pos.gui.show_popup('dialog', {
                                title: 'Warning',
                                body: 'Your pos config have not allow you manual choice pricelist, contact your admin and check to checkbox: Go to POS config / Tab [Order and Booking] Allow cashiers select pricelist',
                            })
                        } else {
        ...
        show: function () {
            var self = this;
            this._super();
            this.product_categories_widget = new core_screen.product_category_list(this, {
                pos_categories: this.pos.pos_categories,
            });
            this.pos.bind('refresh:categories', function () {
                if (self.pos.config.mobile_responsive) {
                    self.product_categories_widget = new core_screen.product_category_list(self, {
                        pos_categories: self.pos.pos_categories,
                    });
                    var $el_categories_list;
                    if (self.pos.config.mobile_responsive) {
                        $el_categories_list = $('.categories_list')
                    } else {
                        $el_categories_list = self.$('.categories_list')
                    }
                    self.product_categories_widget.replace($el_categories_list);
                }
            }, self);
            if (!this.pos.config.mobile_responsive) {
                ...
                this.$('.categories_list').css('width', '0%');
                this.$('.product-list-scroller').css('width', '100%');
            } else {
                ...
                if (this.pos.config.mobile_responsive) {
                    ...
                    $el_categories_list = this.$('.categories_list')
                }
                this.product_categories_widget.replace($el_categories_list);
    ...
    screens.ProductListWidget.include({
            ...
            this.pos.bind('sync:product', function (product_data) { // product screen update screen
                ...
                if ([11, 12].indexOf(self.pos.server_version) != -1) {
                    ...
                    self.pos.db.add_products(_.map([product_data], function (product) {
                        ...
                        var current_pricelist = self._get_default_pricelist();
        ...
        renderElement: function () {
            ...
            this.pos.trigger('refresh:categories');
