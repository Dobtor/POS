...
odoo.define('pos_retail.promotion', function (require) {
    ...
    models.Order = models.Order.extend({
        ...
        compute_discount_total_order: function (promotion) { // 1. compute discount filter by total order
            ...
            */
            var can_apply = this.checking_apply_total_order(promotion);
            if (!can_apply) {
                return;
            }
            var discount_lines = this.pos.promotion_discount_order_by_promotion_id[promotion.id];
        ...
        get_promotions_active: function () {
            ...
            for (var i = 0; i < this.pos.promotions.length; i++) {
                ...
                if (promotion['type'] == '1_discount_total_order' && this.checking_apply_total_order(promotion) && this.checking_special_client(promotion)) {
                    can_apply = true;
                    promotions_active.push(promotion);
                } else if (promotion['type'] == '2_discount_category' && this.checking_can_discount_by_categories(promotion) && this.checking_special_client(promotion)) {
                    can_apply = true;
                    promotions_active.push(promotion);
                } else if (promotion['type'] == '3_discount_by_quantity_of_product' && this.checking_apply_discount_filter_by_quantity_of_product(promotion) && this.checking_special_client(promotion)) {
                    can_apply = true;
                    promotions_active.push(promotion);
                } else if (promotion['type'] == '4_pack_discount' && this.checking_special_client(promotion)) {
                    var promotion_condition_items = this.pos.promotion_discount_condition_by_promotion_id[promotion.id];
                ...
                } else if (promotion['type'] == '5_pack_free_gift' && this.checking_special_client(promotion)) {
                    var promotion_condition_items = this.pos.promotion_gift_condition_by_promotion_id[promotion.id];
                    var checking_pack_discount_and_pack_free = this.checking_pack_discount_and_pack_free_gift(promotion_condition_items);
                    if (checking_pack_discount_and_pack_free) {
                ...
                } else if (promotion['type'] == '6_price_filter_quantity' && this.checking_apply_price_filter_by_quantity_of_product(promotion) && this.checking_special_client(promotion)) { // sale off
                    can_apply = true;
                    promotions_active.push(promotion);
                } else if (promotion['type'] == '7_special_category' && this.checking_apply_specical_category(promotion) && this.checking_special_client(promotion)) {
                    can_apply = true;
                    promotions_active.push(promotion);
                } else if (promotion['type'] == '8_discount_lowest_price' && this.checking_special_client(promotion)) {
                    can_apply = true;
                    promotions_active.push(promotion);
                } else if (promotion['type'] == '9_multi_buy' && this.checking_special_client(promotion)) {
                    var check_multi_by = this.checking_multi_by(promotion);
                    if (check_multi_by) {
                        can_apply = check_multi_by;
                        promotions_active.push(promotion);
                    }
                } else if (promotion['type'] == '10_buy_x_get_another_free' && this.checking_special_client(promotion)) {
                ...
                } else if (promotion['type'] == '11_first_order' && this.checking_special_client(promotion)) {
                    var can_apply_promotion = this.checking_first_order_of_customer();
                    if (can_apply_promotion) {
