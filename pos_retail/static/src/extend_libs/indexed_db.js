                ...
                if (sequence < max_sequence) {
                    $.when(self.search_read('product.product', sequence)).then(function (products) {
                        if (products.length > 0) {
                            ...
                            if (self.pos.total_products <= self.pos.config.max_products_render) {
                                ...
                                self.pos.chrome.loading_message(_t('Restored products total rows: ' + self.pos.total_products));
                            } else {
                                sequence = max_sequence;
                                self.pos.limited_size = true;
                                done.resolve(products);
                                console.warn('Limited size, total products bigger than 500,000 rows');
                            }
                        }
                    }).done(function () {
                        sequence += 1;
                ...
                if (sequence > max_sequence) {
                    ...
                    $.when(self.search_read('res.partner', sequence)).then(function (results) {
                        if (results.length > 0) {
                            ...
                            self.pos.chrome.loading_message(_t('Restored customers total rows: ' + self.pos.total_clients));
