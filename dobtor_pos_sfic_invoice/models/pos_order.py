...
class PosOrder(models.Model):
    ...
    bill_journal = fields.Many2one('account.journal', related='session_id.config_id.bill_journal_id', string='Bill Journal', store=True, readonly=True)
    bill_account_move = fields.Many2one('account.move', string='Journal Entry (BILL)', readonly=True, copy=False)
    ...
    def _reconcile_payments(self):
        for order in self:
            ...
            aml = aml.filtered(lambda r: not r.reconciled and r.account_id.internal_type ==
                               'receivable' and r.partner_id == order.partner_id.commercial_partner_id)
            try:
                aml.reconcile()
            except Exception:
                _logger.exception(
                    'Reconciliation did not work for order %s (Invoice)', order.name)
    ...
    #                     })
    #     order = None
    #     for order in self.filtered(lambda o: not o.account_move or (o.state == 'paid' and o.invoice_state != 'invoiced' and not o.returned_order)):
    #         print('--------------------')

    #         if order.returned_order:
    #             print('------ return order --------')
    #             current_company = order.bill_journal.company_id
    ...
    #                 journal_id = order.bill_journal.id
